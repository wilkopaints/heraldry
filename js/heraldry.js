...
function getArrangement(count, shape) {
    const arr = {
        0: { positions: [], size: 0 },
        1: { positions: [{ cx: 100, cy: 115 }], size: 145 },
        2: { positions: [{ cx: 60, cy: 95 }, { cx: 140, cy: 95 }], size: 75 },
        3: { positions: [{ cx: 60, cy: 65 }, { cx: 140, cy: 65 }, { cx: 100, cy: 155 }], size: 70 },
        4: { positions: [{ cx: 60, cy: 55 }, { cx: 140, cy: 55 }, { cx: 60, cy: 135 }, { cx: 140, cy: 135 }], size: 62 },
        5: { positions: [{ cx: 60, cy: 55 }, { cx: 140, cy: 55 }, { cx: 60, cy: 120 }, { cx: 140, cy: 120 }, { cx: 100, cy: 180 }], size: 57 },
        6: { positions: [{ cx: 60, cy: 40 }, { cx: 140, cy: 40 }, { cx: 60, cy: 95 }, { cx: 140, cy: 95 }, { cx: 60, cy: 150 }, { cx: 140, cy: 150 }], size: 52 },
    };

    if (shape === 'quarterly') {
        if (count === 3) return { positions: [{ cx: 50, cy: 55 }, { cx: 150, cy: 55 }, { cx: 100, cy: 165 }], size: 70 };
        if (count === 4) return { positions: [{ cx: 50, cy: 55 }, { cx: 150, cy: 55 }, { cx: 50, cy: 165 }, { cx: 150, cy: 165 }], size: 62 };
    } else if (shape === 'partyPerFess') {
        if (count === 2) return { positions: [{ cx: 100, cy: 55 }, { cx: 100, cy: 165 }], size: 75 };
        if (count === 3) return { positions: [{ cx: 60, cy: 55 }, { cx: 140, cy: 55 }, { cx: 100, cy: 165 }], size: 70 };
    } else if (shape === 'partyPerPale') {
        if (count === 2) return { positions: [{ cx: 50, cy: 115 }, { cx: 150, cy: 115 }], size: 75 };
    } else if (shape === 'chief') {
        if (count === 1) return { positions: [{ cx: 100, cy: 30 }], size: 55 };
        if (count === 2) return { positions: [{ cx: 60, cy: 30 }, { cx: 140, cy: 30 }], size: 45 };
        if (count === 3) return { positions: [{ cx: 40, cy: 30 }, { cx: 100, cy: 30 }, { cx: 160, cy: 30 }], size: 40 };
    } else if (shape === 'cross') {
        if (count === 5) return { positions: [{ cx: 100, cy: 108 }, { cx: 100, cy: 45 }, { cx: 100, cy: 175 }, { cx: 40, cy: 108 }, { cx: 160, cy: 108 }], size: 45 };
    } else if (shape === 'saltire') {
        if (count === 5) return { positions: [{ cx: 100, cy: 108 }, { cx: 50, cy: 48 }, { cx: 150, cy: 48 }, { cx: 50, cy: 168 }, { cx: 150, cy: 168 }], size: 45 };
    } else if (shape === 'bend') {
        if (count === 2) return { positions: [{ cx: 70, cy: 70 }, { cx: 130, cy: 145 }], size: 60 };
        if (count === 3) return { positions: [{ cx: 55, cy: 52 }, { cx: 100, cy: 108 }, { cx: 145, cy: 164 }], size: 50 };
    } else if (shape === 'bendSinister') {
        if (count === 2) return { positions: [{ cx: 70, cy: 145 }, { cx: 130, cy: 70 }], size: 60 };
        if (count === 3) return { positions: [{ cx: 55, cy: 164 }, { cx: 100, cy: 108 }, { cx: 145, cy: 52 }], size: 50 };
    }
...
function getFieldColourAt(x, y, shape, col1, col2) {
    ...
    switch (shape) {
        ...
        case 'chevron': {
            if (x < 100) { const s = x + y; return (s >= 120 && s <= 160) ? col2 : col1; }
            else { const d = x - y; return (d >= 40 && d <= 80) ? col2 : col1; }
        }
        case 'cross': return ((x >= 75 && x <= 125) || (y >= 83 && y <= 133)) ? col2 : col1;
        case 'saltire': return (Math.abs(y - 1.2 * x + 12) < 30 || Math.abs(y + 1.2 * x - 228) < 30) ? col2 : col1;
        case 'pall': {
        ...
        case 'bordure': {
            ...
            if (y >= 110 && ((x - 100) ** 2 / 6400 + (y - 160) ** 2 / 4000) > 1) return col2;
            return col1;
        }
...
function renderFromControls() {
    ...
    const { positions, size } = getArrangement(count, shape);
